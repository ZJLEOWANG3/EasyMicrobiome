---
title: "Microbiome Analysis for Zhou16S new sample"
author: "Zijian Wang"
date: "2/27/2023"
output: pdf_document
---
...
# set name
tax.level = "phylum" # phylum, genus
sort.tf = "sort" # sort, unsort, all
if (sort.tf=="sort"){
  split = c(seq(1,32,4),seq(2,32,4)) # sorted
} else if (sort.tf=="unsort"){
  split = c(seq(3,32,4),seq(4,32,4)) # unsorted
} else if (sort.tf=="all"){
  split=FALSE
}
pathlist = list(abundance=paste("../data/ZhouSoil16S/New/abundance/",
                                tax.level,".tsv",sep=""),
                meta="../data/ZhouSoil16S/New/metadata_new.xlsx")
projlist = list(projname = paste("ZhouSoil16S_New_",sort.tf,"_",tax.level,sep=""),
                groupcol = c("set.name","control","warming","half.precipitation",
                             "double.precipitation","unclipped"),
                sheetindex = 1, 
                endRow = 33) # first sheet

# excluded column names cor CCA and Env. Correlation
exclude.cca = c("set.name","control","warming","half.precipitation",
                "double.precipitation","unclipped","sorted")
...
for (temp.col in projlist$groupcol){
  ...
  print(p)
  width = 3/2 * length(unique(p$data$measure)) * length(unique(p$data[,4])) 
  ggsave(paste('../output/',projlist$projname,"/",'anova_div_',temp.col,'.png',sep=""),p,height=5,width=width)
  }
```
...
# change line 49 for font size
# for all conditions, it shall have levels > 2

for (gp in projlist$groupcol){
  print(gp)
  pcca <- tryCatch(plot_cca(physeq = physeq, grouping_column = gp, pvalueCutoff = 1, 
      env.variables = NULL, num.env.variables = NULL, 
      exclude.variables = exclude.cca, 
      draw_species = F), 
      error = function(e) {
...
# to.be.exclude.group = exclude.cca.corr[-which(gp == exclude.cca)]
# SAM2 = SAM2[,!(names(SAM2)) %in% to.be.exclude.group]
# physeq2 <- phyloseq(SAM2,otu_table(physeq))

env.taxa.cor <- taxa.env.correlation(physeq, 
                                     grouping_column= gp,
...
TP..mg.L. ~ warming + half.precipitation + double.precipitation + unclipped
NO3..mg.L. ~ warming + half.precipitation + double.precipitation + unclipped
NH4..mg.L. ~ warming + half.precipitation + double.precipitation + unclipped
Phosphate..mg.L. ~ warming + half.precipitation + double.precipitation + unclipped
H2O..mg.gsoil. ~ warming + half.precipitation + double.precipitation + unclipped

Richness ~ NH4..mg.L. + NO3..mg.L. + Phosphate..mg.L. + H2O..mg.gsoil. + TP..mg.L.
Biomass..mg.gsoil. ~ NH4..mg.L. + NO3..mg.L. + Phosphate..mg.L. + H2O..mg.gsoil. + TP..mg.L.

# AP not only affected by soil but also PAO microbial structure
AP.Activity..nmol.MUB..g.dry.soil...hour. ~ NH4..mg.L. + NO3..mg.L. + Phosphate..mg.L. + H2O..mg.gsoil. + TP..mg.L. + Richness + Biomass..mg.gsoil.


# covariance # if split
# Normalized.PCO..FACS ~~ Normalized.PCO..Raman
# Normalized.PAO..FACS ~~ Normalized.PAO..Raman
# Normalized.PHAAO..FACS ~~ Normalized.PHAAO..Raman
'
