---
title: "Microbiome Analysis for Winter Wheat 2022 Summer"
author: "Zijian Wang"
date: "4/18/2023"
output: pdf_document
---
...
tax.level = "genus" # genus, phylum
# set name - Full dataset
pathlist = list(abundance=paste("../data/CROPPS2022Summer16S/abundance/",
                tax.level,".tsv",sep=""),
                meta="../data/CROPPS2022Summer16S/metadata_wheat_v3.xlsx")
projlist = list(projname = paste("Winter_Wheat_",tax.level,sep=""),
                groupcol = c("Timepoint","Plot..Name."), #all () and space will become .
                sheetindex = 1, # first sheet
                endRow = 32) # how many rows within the metadata excel

exclude.env.corr = c("Timepoint","Plot..Name.","Wet","Normal") # to be deleted
```
...
method = c("richness","simpson","shannon")
for (temp.col in projlist$groupcol){
  ...
  print(p)
  ggsave(paste('../output/',projlist$projname,"/",'anova_div_',temp.col,'.png',sep=""),p,height=5,width=3*length(method))
...
print(p)
ggsave(paste('../output/',projlist$projname,"/",'env.corr.strain.',gp,'.png',sep=""),p,height=10,width=1.3*length(colnames(physeq2@sam_data)))}}
```
